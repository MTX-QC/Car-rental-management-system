...
public Result<Book> findById(String id) {
    try {
        ...
        if(book==null){
            return new Result(false,"查询车辆失败！");
        }
        return new Result(true,"查询车辆成功",book);
    }catch (Exception e){
        e.printStackTrace();
        return new Result(false,"查询车辆失败！");
    }
...
public Result borrowBook(Book book, HttpSession session) {
    ...
    try {
        ...
        if (count != 1) {
            return new Result(false, "租借车辆失败!");
        }
        return new Result(true, "租借成功，请到MTX车辆中心取车!");
    } catch (Exception e) {
        e.printStackTrace();
        return new Result(false, "租借车辆失败!");
    }
...
public Result returnBook(String id, HttpSession session) {
    ...
    try {
        ...
        if (!flag) {
            return new Result(false, "还车失败!");
        }
        return new Result(true, "还车确认中，请把车开到MTX车辆中心归还!");
    }catch (Exception e){
        e.printStackTrace();
        return new Result(false, "还车失败!");
    }
