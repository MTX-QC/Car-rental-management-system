...
public class UserController {
    ...
    */
    ...
    public String login(User user, HttpServletRequest request) {
        try {
            ...
            if (u != null) {
                ...
                } else if ("USER-PLUS".equals(role)) {
                    return "redirect:/admin/index-plus.jsp";
                } else if ("USER-Check".equals(role)) {
                    return "redirect:/admin/index_check.jsp";
                } else {
    ...
    //转为VIP
    @RequestMapping("/updateToVip")
    public String updateToVip(HttpSession session) {
        //获取当前登录的用户ID
        Integer id = ((User) session.getAttribute("USER_SESSION")).getId();
        userService.updateToVip(id);
        return "redirect:/admin/index_check.jsp";
    }
    /*
    注销登录
     ...
     * @param pageSize 数据列表1页展示多少条数据
     */
    @RequestMapping("/queryAuditUser")
    public ModelAndView queryAuditUser(User user, Integer pageNum, Integer pageSize) {
        if (pageNum == null) {
            pageNum = 1;
        }
        if (pageSize == null) {
        ...
        PageResult pageResult = userService.queryAuditUser(user, pageNum, pageSize);
        ModelAndView modelAndView = new ModelAndView();
        modelAndView.setViewName("user_check");
        modelAndView.addObject("pageResult", pageResult);
        modelAndView.addObject("search", user);
        modelAndView.addObject("pageNum", pageNum);
        modelAndView.addObject("gourl", "/user/queryAuditUser");
        return modelAndView;
    }
    ...
    public Result auditUser(Integer id) {
        try {
