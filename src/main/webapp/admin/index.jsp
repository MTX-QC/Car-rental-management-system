                    ...
                    <ul class="nav navbar-nav">
                        <%--<c:if test="${USER_SESSION.role =='USER'}">--%>
                            <li class="dropdown user user-menu">
                                <a href="${pageContext.request.contextPath}/user/updateToVip?userId=${loggedInUser.userId}">
                                    <span class="hidden-xs">升级为VIP</span>
                                </a>
                            </li>
<%--                        </c:if>--%>
<%--                        <c:if test="${USER_SESSION.role =='USER-Check'}">
                            <li class="dropdown user user-menu">
                                <span class="hidden-xs">审核中</span>
                            </li>
                        </c:if>--%>
                            <%--<c:if test="${USER_SESSION.role =='USER' && USER_SESSION.role != 'USER-Check'}">
                                <!-- 只有当 USER_SESSION.role 的值为 'USER' 且不为 'USER-Check' 时，才会显示以下内容 -->
                                <li class="dropdown user user-menu">
                                    <a href="${pageContext.request.contextPath}/user/updateToVip?userId=${loggedInUser.userId}">
                                        <span class="hidden-xs">升级为VIP</span>
                                    </a>
                            ...
                            <c:if test="${USER_SESSION.role =='USER-Check'}">
                                <!-- 只有当 USER_SESSION.role 的值为 'USER-Check' 时，才会显示以下内容 -->
                                <li class="dropdown user user-menu">
                                    <span class="hidden-xs">审核中</span>
                                </li>
                            </c:if>--%>
